How can I contribute to super_sac?
============================================
...
There are lots of ways you can contribute to super_sac:
...
super_sac can remain robust and reliable thanks to users who notify us of
the problems they encounter. So thank you for [reporting an issue](https://github.com/QData/super_sac/issues).
...
`super_sac`. `git` is not the easiest tool to use but it has the greatest
manual. Type `git --help` in a shell and enjoy. If you prefer books, [Pro
Git](https://git-scm.com/book/en/v2) is a very good reference.

Follow these steps to start contributing:

1. Fork the [repository](https://github.com/QData/super_sac) by
...
2. Clone your fork to your local disk, and add the base repository as a remote:
   ...
   ```bash
   $ git clone git@github.com:<your Github handle>/super_sac.git
   $ cd super_sac
   $ git remote add upstream https://github.com/QData/super_sac
   ```
...
4. Set up a development environment by running the following commands in a virtual environment:
   ...
   This will install `super_sac` in editable mode and install `black` and 
   `isort`, packages we use for code formatting.
   
   (If super_sac was already installed in the virtual environment, remove
   it with `pip uninstall super_sac` before reinstalling it in editable
...
5. Develop the features on your branch.
   ...
   `super_sac` relies on `black` and `isort` to format its source code
...
You can run super_sac tests with `pytest`. Just type `make test`.
